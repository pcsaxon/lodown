...
_.contains = function(array, value){
    ...
    while(i <= array.length){
      result = (value === array[i]) ? true : (i == array.length) ? false : nextItem(i);
    }
    return result;
};
...
_.map = function(col, func){
    ...
    if(Array.isArray(col)){
        for(var i = 0; i < col.length; i++){
            let newValue = func(col[i], i, col);
            toReturn.push(newValue);
        }
    }
    else{
        for(var key in col){
            let newValue = func(col[key], key, col);
            toReturn.push(newValue);
        }
    }
    return toReturn;
...
_.every = function(col, func){
  ...
  return(falsey.length > 0) ? false : true;
  };
...
_.some = function(col, func){
  ...
  return(truthy.length > 0) ? true : false;
};
